...
load("cpm7.RData")
load("cpm8.RData")
cpm_data=rbind(cpm1,cpm2,cpm3,cpm4,cpm5,cpm6,cpm7,cpm8)
col_names = colnames(dataClin)
vars <- rownames(cpm_data)
cpm_fixed = as.data.frame(t(cpm_data))
rownames(cpm_fixed) = stringr::str_replace_all(rownames(cpm_fixed),"\\.",'-')
...
server <- function(input, output) {
    ...
    output$summary <- renderPrint({
        cor.test(t(cpm_data)[,input$xcol], t(cpm_data)[,input$ycol], method="pearson")
    })
